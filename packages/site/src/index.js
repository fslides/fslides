...
async function serveDeck(request, url, env) {
  ...
  if (parts.join('/') && decodeURIComponent(parts.join('/')).includes('..')) return new Response('Bad path', { status: 400, headers: NO_STORE });
  ...
  // /owner/repo/obs-preso/slide.html → deckSubPath='obs-preso', key='slide.html'
  ...
  if (!loaded) {
    ...
    if (!deckSubPath && !key) {
      ...
      if (manifest && Array.isArray(manifest.decks) && manifest.decks.length > 0) {
        ...
        return Response.redirect(`${url.origin}/${owner}/${repo}/${first}/`, 302);
      }
    }
